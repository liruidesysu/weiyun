...
def delete_floating_ip(id):
    ...
    try:
        ...
        LOG.exception("Can not delete floating ip by id:%s" % id)
        session.rollback()
        raise
    finally:
        session.close()
...
def delete_floating_ip_address(ip_address):
    """根据 IP 地址删除 Floating IP 缓存"""
    session = get_session()
    try:
        count = session.query(FloatingIp).filter(
            FloatingIp.ip_address == ip_address).delete()
        session.commit()
    except:
        LOG.exception("Can not delete floating ip by ip_address:%s" % ip_address)
        session.rollback()
        raise
    finally:
        session.close()
...
def allocate_floating_ip(external_network_id):
    ...
    try:
        ...
        if ip:
            ...
            ip_id = ip.id
            ip = session.query(FloatingIp).filter(FloatingIp.id == ip_id).first()
            session.expunge(ip)
    except:
        LOG.exception("Can not allocate local floating ip by external_network_id:%s" % external_network_id)
        session.rollback()
        raise
    finally:
        session.close()
...
def allocate_floating_ip_by_ref_id(fip_ref_id):
    ...
    try:
        ...
        LOG.exception("Can not allocate local floating ip by fip_ref_id:%s" % fip_ref_id)
        session.rollback()
        raise
    finally:
        session.close()
...
def reclaim_floating_ip(ip_address):
    ...
    try:
        ...
        LOG.exception("Can not reclaim local floating ip by ip_address:%s" % ip_address)
        session.rollback()
        raise
    finally:
        session.close()
...
def delete_floating_ip_by_ref_id(fip_ref_id):
    ...
    try:
        ...
        LOG.exception("Can not delete floating ip by fip_ref_id:%s" % fip_ref_id)
        session.rollback()
        raise
    finally:
        session.close()